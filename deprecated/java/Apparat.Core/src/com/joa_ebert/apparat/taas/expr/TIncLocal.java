 ...
 * 
 */
public class TIncLocal extends AbstractLocalExpr
{
	@TaasReference
	public TaasLocal source;

	public TIncLocal( final TaasLocal local )
	{
		super( local );
			...
			final MethodBody body, final Bytecode code )
	{
		...
		{
			op = new IncLocalInt( local.getIndex() );
		}
		else
		{
			op = new IncLocal( local.getIndex() );
		}
	...
	{
		return "[TIncLocal " + local.toString() + " = " + source.toString()
				+ " + 1]";
	}
}
