...
final class PrintfFormat
{
     ...
     */
		...
		private String printOFormat( final int x )
		{
			...
			{
				final String t = Integer.toString( ( ~( -x - 1 ) )
						^ Integer.MIN_VALUE, 8 );
				switch( t.length() )
				{
					case 1:
		...
		private String printOFormat( final long x )
		{
			...
			{
				final String t = Long.toString( ( ~( -x - 1 ) )
						^ Long.MIN_VALUE, 8 );
				switch( t.length() )
				{
					case 1:
		...
		private String printXFormat( final short x )
		{
			...
			else if( x < 0 )
			{
				...
				{
					t = Integer
							.toString( ( ~( -x - 1 ) ) ^ Short.MIN_VALUE, 16 );
	...
	public String sprintf( final Object x ) throws IllegalArgumentException
	{
		...
		while( e.hasMoreElements() )
		{
			...
			else
			{
				...
				{
					sb
							.append( cs.internalsprintf( ( (Double)x )
									.doubleValue() ) );
				}
		...
		while( e.hasMoreElements() )
		{
			...
			else
			{
				...
				else
				{
					...
					{
						cs
								.setFieldWidthWithArg( ( (Integer)o[ i ] )
										.intValue() );
						i++;
					}
				...
				{
					sb.append( cs
							.internalsprintf( ( (Byte)o[ i ] ).byteValue() ) );
				}
				...
				{
					sb.append( cs
							.internalsprintf( ( (Long)o[ i ] ).longValue() ) );
				}
